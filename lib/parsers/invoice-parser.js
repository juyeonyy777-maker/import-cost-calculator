...
function findCostAmount(text, itemNames) {
  let totalKrw = 0, totalVat = 0, foundRate = 0, foundCurrency = '';
  let found = false;
  ...
  for (const name of itemNames) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (/CNY/i.test(after)) {
        ...
        if (parsed) { totalKrw += parsed.krwTotal; totalVat += parsed.vatAmount; if (parsed.exchangeRate > 0) foundRate = parsed.exchangeRate; foundCurrency = 'CNY'; found = true; continue; }
      }
      if (/USD/i.test(after)) {
        const parsed = parseUsdLine(after.replace(/^.*?USD/i, ''));
        if (parsed) { totalKrw += parsed.krwTotal; totalVat += parsed.vatAmount; if (parsed.exchangeRate > 0) foundRate = parsed.exchangeRate; foundCurrency = 'USD'; found = true; continue; }
      }
      ...
      totalKrw += nums.length >= 2 ? nums[1] : nums[0];
      totalVat += nums.length >= 3 ? nums[2] : 0;
      foundCurrency = 'KRW';
      found = true;
    }
  }
  ...
  return { krwTotal: totalKrw, vatAmount: totalVat, exchangeRate: foundRate, currency: foundCurrency };
}
