...
function parseExcel(buffer) {
  ...
  // 박스 내 CBM을 totalAmount 비율로 배분
  for (const [, indices] of Object.entries(boxItems)) {
    const boxCbm = items[indices[0]].boxCbm;
    const boxTotal = indices.reduce((sum, idx) => sum + items[idx].totalAmount, 0);
    for (const idx of indices) {
      items[idx].allocatedCbm = boxTotal > 0
        ? boxCbm * (items[idx].totalAmount / boxTotal)
        : boxCbm / indices.length;
    }
  }
  ...
  for (const item of items) {
    if (!skuMap[item.sku]) {
      skuMap[item.sku] = {
        sku: item.sku, productName: item.productName, option: item.option,
